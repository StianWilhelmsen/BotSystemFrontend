    ...
    <div class="user-fines-container">
      ...
      >
        <div class="user-details">
          <div class="user-name">
            {{ userWithFine.user.firstname }} {{ userWithFine.user.lastname }}
          </div>
          <div class="user-fine">{{ userWithFine.totalWeight }}</div>
        </div>
        <div v-if="expandedIndex === index" class="fine-details">
          <div
            v-if="loadingStates.get(userWithFine.user.id)"
            class="spinner"
          ></div>
          <!-- Loading spinner -->
          <div
            v-else
            v-for="(fine, fIndex) in userFinesDetails.get(userWithFine.user.id)"
            :key="fIndex"
            class="fine-item"
            @click.stop="toggleFineDetails(fine)"
          >
            <span
              ...
              <div class="status-container">
                <div class="approved-status">
                  ...
                  <div v-else class="title">
                    <button
                      class="defence-button"
                      @click.prevent.stop="approveFine(fine)"
                    >
                      Godkjenn
                    </button>
                  </div>
                ...
                <div class="paid-status">
                  ...
                  <div v-else class="title">
                    <button
                      class="defence-button"
                      v-if="!fine.paid"
                      @click.prevent.stop="payFine(fine)"
                    >
                      Betalt
                    </button>
                  </div>
            ...
            <div
              v-if="!userFinesDetails.get(userWithFine.user.id)"
              class="fine-details-placeholder"
            ></div>
          </div>
        </div>
      </div>
...
async function approveFine(fine) {
  try {
    const response = await axios.put(
      `${apiPort.value}/api/fine/${fine.id}/approve`,
    );
...
async function payFine(fine) {
  try {
    const response = await axios.put(
      `${apiPort.value}/api/fine/${fine.id}/pay`,
    );
...
async function retrieveUsersWithFines() {
  try {
    const response = await axios.get(
      apiPort.value + "/api/users/getUsersWithFines",
    );
...
async function toggleExpansion(index) {
  ...
  if (expandedIndex.value !== index) {
    ...
    if (!userFinesDetails.has(userId)) {
      ...
      try {
        const response = await axios.get(
          `${apiPort.value}/api/fine/${userId}/fines`,
        );
